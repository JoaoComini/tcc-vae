...
x_train = MinMaxScaler().fit_transform(full_data.drop(columns=["class"]))
y_train = full_data['class'].replace({
            'warezmaster' : 'R2L',
            'warezclient' : 'R2L',
            'teardrop' : 'DoS',
            'spy' : 'R2L',
...
fig, ax = plt.subplots()
cax = ax.scatter(encoded_data[:, 0], encoded_data[:, 1], c=y_train.cat.codes, cmap=cmap)
